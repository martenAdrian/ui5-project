...
class ProjectPreprocessor {
	...
	*/
	async processTree(tree) {
		...
		while (queue.length) {
			const {
				...
				parent,
				level
			} = queue.shift(); // Get and remove first entry from queue
	...
	async loadProjectConfiguration(project) {
		...
		let configPath = project.configPath /* || path.join(project.path, "/ui5.yaml") */ ;
		var aResults = await Promise.allSettled([{
			type: 'customPath',
			path: configPath
		}, {
			type: 'yamlPath',
			path: path.join(project.path, "/ui5.yaml")
		}, {
			type: 'jsonPath',
			path: path.join(project.path, "/ui5.json")
		}].map((oCheck) => {
			return new Promise((resolve, reject) => {
				if (fs.existsSync(oCheck.path)) {
					resolve(oCheck);
				} else {
					reject();
				}
		...
		aResults = aResults.filter(oRes => oRes.status === "fulfilled");
		if (aResults.length === 1) {
			configPath = aResults[0].value.path;
		} else if (aResults.length > 1) {
			aResults.sort((a, b) => {
				if (a.value.type === "customPath") {
					return -1;
				}
				if (a.value.type === "yamlPath" && b.value.type === "jsonPath") {
					return -1;
				}
				if (a.value.type !== "customPath" && b.value.type === "customPath") {
					return 1;
				}
				if (a.value.type === "jsonPath" && b.value.type === "yamlPath") {
					return 1;
				}
			});
			configPath = aResults[0].value.path;
		...
		try {
			...
			if (!configPath) {
				configPath = path.join(project.path, "/ui5[.yaml, .json]");
			}
