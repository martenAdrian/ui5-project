...
class ProjectPreprocessor {
	...
	*/
	...
	async loadProjectConfiguration(project) {
		...
		if (!configPath) {
			if (!await access(path.join(project.path, "/ui5.yaml"), mFSConsts.F_OK)) {
				configPath = path.join(project.path, "/ui5.yaml");
			};

			if (!await access(path.join(project.path, "/ui5.json"), mFSConsts.F_OK)) {
				configPath = path.join(project.path, "/ui5.json");
			};
        }
